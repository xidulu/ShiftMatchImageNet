# Match scaled input using a subset of training statistics
...
levels = args.level
print(f'Evaluating on level: {levels} with {train_set_size} training samples')
...
for level in levels:
    for noise_type in CORRUPTIONS:
        ...
        with open(f'./cov_cache/bn_sm_{train_set_size}.pkl', "rb") as f:
            sm_state = pickle.load(f)
        input_sm_func = match_channel_sep(train_set_size)
        correct = 0
        total = 0
        labels = []
        predss = []
        ...
        jnp.save(f'./logs/sm_small_{noise_type}_{level}_preds_{train_set_size}.pkl', predss)
        jnp.save(f'./logs/sm_small_{noise_type}_{level}_label_{train_set_size}.pkl', labels)
